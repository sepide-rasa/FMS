...
namespace NewFMS.Areas.PayRoll.Controllers
{
    public class VamController : Controller
    {
        ...
        public ActionResult Index(string containerId)
        {
            ...
            ViewData.Model = new NewFMS.Models.Vam(); ;
        ...
        {//باز شدن پنجره
            ...
            if (id == 0)
            {
                var q = servic.GetVamWithFilter("PersonalId", PersonalId.ToString(),Convert.ToInt32(Session["OrganId"]), 0, IP, out Err).Where(k => k.fldStatus == true).FirstOrDefault();
                if (q != null)
                    HaveVam = 1;
            }
            if (HaveVam == 0)
        ...
        public ActionResult Save(WCF_PayRoll.OBJ_Vam Vam,int OrganId)
        {
            if (Session["Username"] == null)
                return RedirectToAction("logon", "Account", new { area = "" });
            ...
            try
            {
                ...
                if (Vam.fldId == 0)
                { //ذخیره
                    Msg = servic.InsertVam(Vam.fldPersonalId, Vam.fldTarikhDaryaft, Vam.fldTypeVam, Vam.fldMablaghVam,Vam.fldStartDate, Vam.fldCount,
                        Vam.fldMablagh, Vam.fldMablaghVam, true, Vam.fldDesc, Session["Username"].ToString(), (Session["Password"].ToString()),
                        OrganId/*Convert.ToInt32(Session["OrganId"])*/, IP, out Err);
                        MsgTitle = "ذخیره موفق";
                }
                else
                { //ویرایش
                    Msg = servic.UpdateVam(Vam.fldId, Vam.fldPersonalId, Vam.fldTarikhDaryaft, Vam.fldTypeVam, Vam.fldMablaghVam, Vam.fldStartDate, Vam.fldCount,
                    Vam.fldMablagh, Vam.fldMablaghVam, true, Vam.fldDesc, Session["Username"].ToString(), (Session["Password"].ToString()),
                    OrganId/*Convert.ToInt32(Session["OrganId"])*/, IP, out Err);
        ...
        public ActionResult Delete(int id,int OrganId)
        {
            ...
            {
                //حذف
                servic.DeleteVam(id, Session["Username"].ToString(), (Session["Password"].ToString()), OrganId/*Convert.ToInt32(Session["OrganId"])*/, IP, out Err);
        ...
        public ActionResult Tasvie(int id, int OrganId)
        {
            if (Session["Username"] == null)
                return RedirectToAction("logon", "Account", new { area = "" });
            string Msg = "", MsgTitle = ""; var Er = 0;
            ...
            {
                //حذف
                servic.VamStatusUpdate(id, false, Session["Username"].ToString(), (Session["Password"].ToString()), OrganId/*Convert.ToInt32(Session["OrganId"])*/, IP, out Err);
                Msg = "تسویه وام با موفقیت انجام شد.";
                MsgTitle = "تسویه";
                if (Err.ErrorType == true)
                {
                    Msg = Err.ErrorMsg;
        ...
        public ActionResult Details(int Id,int OrganId)
        {
            if (Session["Username"] == null)
                return RedirectToAction("logon", "Account", new { area = "" });
            var q = servic.GetVamDetail(Id,OrganId/* Convert.ToInt32(Session["OrganId"])*/, IP, out Err);
            return Json(new
            {
                fldId = q.fldId,
                fldCount = q.fldCount,
                ...
                fldTypeVam = q.fldTypeVam.ToString(),
                fldDesc = q.fldDesc
            }, JsonRequestBehavior.AllowGet);
        }

        public ActionResult CheckCalc(int Id,int OrganId)
        {
            if (Session["Username"] == null)
                return RedirectToAction("logon", "Account", new { area = "" });
            var q = servic.GetVamWithFilter("CheckCalc", Id.ToString(), OrganId/*Convert.ToInt32(Session["OrganId"])*/, 1, IP, out Err).FirstOrDefault();
            var flag = 0;
            if (q != null)
                flag = 1;
        ...
        public ActionResult Rload(int PersonalId,int OrganId)
        {
            ...
            List<WCF_PayRoll.OBJ_Vam> data = null;

            data = servic.GetVamWithFilter("PersonalId", PersonalId.ToString(), OrganId/*Convert.ToInt32(Session["OrganId"])*/, 100, IP, out Err).ToList();
