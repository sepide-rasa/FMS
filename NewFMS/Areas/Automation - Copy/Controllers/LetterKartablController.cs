    ...
    public class LetterKartablController : Controller
    {
                ...
                try
                {
                    ...
                    for (int j = 0; j < q.Count(); j++)
                    {
                        ...
                        if (q[j].fldBoxTypeID == 1)
                        {
                            ...
                            BoxName[j] = q[j].fldName + "(" + q1.tedad + ")";
                        }
                        else
        ...
        public ActionResult Search(StoreRequestParameters parameters, int BoxId, string ComId,int BoxType,string SearchFilter)    
        {
            ...
            try
            {
                ...
                if (BoxType == 1)
                    ListInbox = servic.SelectInbox("", start, end, BoxId, SearchFilter, Convert.ToInt32(Session["OrganId"]),50, IP, out Err).ToList();
                else if (BoxType == 2)
                    ListSent = servic.SelectSent("", start, end, BoxId, SearchFilter, Convert.ToInt32(Session["OrganId"]), 50, IP, out Err).ToList();
                else if (BoxType == 3)
                    ListSavedLetter = servic.SelectSavedLetter("", start, end, BoxId.ToString(), SearchFilter, Convert.ToInt32(Session["OrganId"]), 50, IP, out Err).ToList();
                else if (BoxType == 4)
                    ListSavedMsg = servic.GetSavedMessageWithFilter("", start, end, BoxId.ToString(), SearchFilter, Convert.ToInt32(Session["OrganId"]), 50, IP, out Err).ToList();
                else if (BoxType == 5)
                    ListDeleted = servic.SelectTrash("", start, end, BoxId, SearchFilter, Convert.ToInt32(Session["OrganId"]), 50, IP, out Err).ToList();
                    ...
                    for (int i = 0; i < letterAssId.Count() - 1; i++)
                    {
                        ...
                        else if (BoxType == 5)//حذف شده ها
                        {
                            ...
                            if (ss[1] == "1")//نامه
                            {
                                ...
                                {
                                    servic.DeleteRonevesht(Convert.ToInt32(ss[0]), Session["Username"].ToString(), (Session["Password"].ToString()), Convert.ToInt32(Session["OrganId"]), IP, out Err);
