        ...
        public ActionResult Save(WCF_Automation.OBJ_Commision Commision)
        {
            ...
            try
            {
                if (Commision.fldDesc == null)
                    Commision.fldDesc = "";
                if (Commision.fldOrganicNumber == null)
                    Commision.fldOrganicNumber = "";
            ...
            if (filterHeaders.Conditions.Count > 0)
            {
                ...
                foreach (var item in filterHeaders.Conditions)
                {
                    ...
                    switch (item.FilterProperty.Name)
                    {
                        ...
                        case "fldName":
                            searchtext = "%" + ConditionValue.Value.ToString() + "%";
                            field = "fldName_haghighi_khososi";
                            break;
                        case "fldFamily":
                            searchtext = "%" + ConditionValue.Value.ToString() + "%";
                            field = "fldFamily_haghighi_khososi";
                            break;
                        case "fldShenase_CodeMeli":
                            searchtext = "%" + ConditionValue.Value.ToString() + "%";
                            field = "fldShenase_CodeMeli_haghighi_khososi";
                            break;
                        case "shomareshabt_father":
                            searchtext = "%" + ConditionValue.Value.ToString() + "%";
                            field = "shomareshabt_father_haghighi_khososi";
                            break;
                    }
                    if (data != null)
